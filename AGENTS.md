...
CuratorX is a **single service**: a Python 3.12 FastAPI backend (`python -m curatorx.web`) that
also serves the pre-built React SPA from `frontend/dist`. Optional stdio/HTTP MCP server
(`python -m curatorx.mcp.server`). There is **no separate database/cache/queue** to run — state is
...
```bash
DATA_DIR=./config PORT=8788 CURATORX_SKIP_DOTENV=1 .venv/bin/python -m curatorx.web
```
Serves on http://127.0.0.1:8788 (`GET /api/health` → `{"status":"ok"}`). `CURATORX_SKIP_DOTENV=1`
...
isolation or as a pair. Cause: the module-global rate limiter in `curatorx/web/rate_limit.py` is not

...
from projectionist.agent.tools import (
    ...
    mark_in_radarr,
    mark_in_sonarr,
    resolve_radarr_search_for_movie,
)
...
register_spa_routes(app, frontend_dist=FRONTEND_DIST, static_dir=STATIC_DIR)
...
def propose_action(payload: Dict[str, Any], user=Depends(get_current_user_dep)) -> Dict[str, Any]:
    ...
    if action == "add_radarr":
        ...
        explicit_search = payload.get("search_for_movie")
        ...
        logger.info(
            "Proposed add_radarr tmdb_id=%s title=%r search_for_movie=%s token=%s",
            payload["tmdb_id"],
            payload.get("title", ""),
            search_for_movie,
            token[:8],
        )
        return {"confirmation_token": token, "search_for_movie": search_for_movie}
...
def admin_radarr_owned_not_indexed(
    limit: int = 50,
    user=Depends(require_role("owner")),
) -> Dict[str, Any]:
...
def admin_radarr_register_existing(

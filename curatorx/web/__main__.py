...
def resolve_host() -> str:
    ...
    ``HOST`` (uvicorn convention) or ``CURATORX_HOST`` to e.g. ``127.0.0.1``.
    """
    host = (os.environ.get("HOST") or os.environ.get("CURATORX_HOST") or "").strip()
...
def warn_if_exposed_without_auth(host: str, port: int) -> None:
    if bind_exposed_without_auth(host, multi_user_enabled=_multi_user_enabled()):
        logger.warning(
            "CuratorX is binding to %s:%s with authentication DISABLED (single-owner "
            "mode): anyone who can reach this port has full admin access. To harden, "
            "set HOST=127.0.0.1, put CuratorX behind an authenticating reverse proxy, "
            "or enable multi-user auth. See SECURITY.md (S3).",
            host,
            port,
        )

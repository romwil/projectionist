...
**Date:** 2026-07-30  
**Status:** Living scorecard (execute hot fixes first, then craft / `/live`)  
...
| Dual watch: Projectionist `/live` gasp UX | **Gap** | Proxy + fullscreen + OSD + CC + EPG + pop-out |
...
| Spec / HELP: dual watch; kill “does not play” | **Partial (1.29.25)** | Spec + HELP flipped; `/live` UX still Gap |
...
0. Collection publish honesty + ID match + modes + async + refill recipe  
0b. Plex sync after publish + logos  
1. Revise product docs/spec (dual watch; gasp `/live`)  
2. `/live` MVP → gasp polish (proxy, fullscreen, OSD, CC, newspaper EPG, pop-out)  

...
When Live Channels is on, Projectionist can publish library-aware stations through Tunarr into **Plex Live TV**. If you already have an OTA antenna / HDHomeRun DVR, keep it — Plex supports multiple tuners. In Admin → Live Channels, use the Plex steps: on **Tuner Setup** select the discovered Tunarr device and enter any US ZIP so Next unlocks (gate only). The **EPG Location** dropdown is commercial lineups only (Fios / DIRECTV / Xfinity / Local Broadcast) — Plex **never** offers XMLTV there, on the first screen or on channel-mapping. Pick any temporary lineup to finish creating the DVR, then open that Tunarr DVR → **DVR Settings** → add/switch **XMLTV** and paste Tunarr’s guide URL. Fake cable channel names until then are expected. Starter stations use channel numbers from 100+ to stay out of typical OTA ranges. Watching stays in Plex; Projectionist does not play live video in-app.

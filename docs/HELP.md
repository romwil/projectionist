...
**Your watch history separates tracked completions from Plex played events.** Open an in-library movie to see **Your watch history**: each completion carries **Certain**, **Likely**, or **Plex played event** evidence and a **Why this count?** explanation. Show pages stay episode-first, with completed episodes, repeat episode completions, and recent activity—never “show watched N times.” Movie cards show a concise tracked-completion count only when Projectionist has evidence tied to your signed-in Plex identity.

> **You ask:** “Did I really rewatch *Zatoichi* three times?”
> The curator prefers your tracked completions when coverage exists, names the confidence behind each one, and keeps the older Plex marked-played count as a labeled fallback when coverage does not exist.

**How this works / honest limit:** Projectionist conservatively joins your played history, progress samples, and terminal events into logical viewings. **Certain** means it observed a completion boundary crossing; **Likely** means it reconstructed one from strong evidence; **Plex played event** means Plex reported played without enough progress detail. None proves uninterrupted playback, attention, or who was in front of the screen, and frequency alone never makes a title a favourite.
...
Plex's movie `viewCount` remains a per-user **completed-or-marked-played event** counter. Organic playback can mark played at a percentage/credits rule, while manual or synced watched actions can affect the same number. Projectionist therefore preserves it as `plex_played_event_count` / the Phase 0 fallback and adds user-scoped `tracked_completions`, `completion_confidence`, `logical_viewings`, `sittings_observed`, `last_tracked_completion_at`, and `tracker_coverage` beside it. Agent policy prefers tracker fields only when coverage is present; otherwise it keeps the older `completed_watches`, `rewatch_count`, `watch_state`, `partial`, and `watch_progress_percent` semantics. TV remains episode-first: show summaries report unique episodes completed, total/repeat episode completions, in-progress episodes, and recent activity—not a fabricated show-level rewatch count.

The confidence model is intentionally conservative: `certain` means a mapped user's logical viewing crossed the 90% tracker boundary in observed progress, `likely` means a terminal completion was reconstructed from monotonic evidence, and `plex_event_only` means Plex emitted a unique played/scrobble/history event without enough progress evidence. Even `certain` does not prove uninterrupted or attentive viewing.
...
Configure outbound mail under **Admin → Mail**. Choose **SMTP** or **Resend**, set from-address / template fields (subject prefix, footer, logo URL), and use **Send test email**. Push the weekly newsletter early (yourself, selected members, or everyone opted in) and generate a Year in Review test reel under **Admin → Newsletters**. Secrets are stored in `settings.json` with mode `0600`, like other API keys.
